...
apply(
    ...
    "        const arisuStarted: any = this.startRandomMotion(\n"
    "          arisuMotion.group,\n"
    "          arisuMotion.priority\n"
    "        );\n"
    "        // Not `>= 0`: a started motion returns its queue entry, an object\n"
    "        // (_motionQueueEntryHandle = this), and `{} >= 0` is false. Every\n"
    "        // gesture that played was counted refused and re-started for its\n"
    "        // whole retry budget. Refusal is the literal -1.\n"
    "        if (arisuStarted !== -1) {\n"
