...
// How much of the screen she covers, as a multiple of the cover fit -- the
// scale at which a square portrait reaches all four edges of a screen that is
// not square, which means the long sides are cropped. It was a contain fit at
// 0.70, which left a third of a 12.9" iPad unused and read as a portrait hung
// on a wall rather than a head in the room. 1.02 rather than 1 because she
// drifts and breathes by a few pixels, and at exactly 1 that motion walks a
// black edge onto the screen. Per character: a portrait cropped tight at the
// chin and one with shoulders in frame do not want the same number.
const FILL = F.fill == null ? 1.02 : F.fill;
...
class Component extends DCLogic {
  ...
  frame(t, now) {
    ...
    const cyp = eink
      ...
      : h * 0.5 + Math.sin(t * 0.47) * 5 * this.dpr * alive - Math.sin(s.nod * Math.PI) * 22 * this.dpr;
